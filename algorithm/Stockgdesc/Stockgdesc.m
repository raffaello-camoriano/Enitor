classdef Stockgdesc < algorithm
    %kpegasos Kernelized  Pegasos algorithm
    ...
    properties
        ...
        lambda      % Lambda used in the Kernelized Pegasos iteration
    ...
    methods
        ...
        function obj = Stockgdesc(map, filter ,varargin)
        ...
        function train(obj , Xtr , Ytr , performanceMeasure , recompute, validationPart , varargin)
            ...
            for i = 1:size(obj.mapParGuesses,2)
                
                mapPar =  obj.mapParGuesses(:,i);                
                
                % Initialize TrainVal kernel
                argin = {};
                argin = [argin , 'mapParGuesses' , full(mapPar)];
                if ~isempty(obj.verbose)
                    argin = [argin , 'verbose' , obj.verbose];
                end                    
                kernelVal = obj.map(Xval,Xtrain, argin{:});            
                kernelVal.next();
                kernelVal.compute(mapPar);
                ...
                filter = obj.filter( obj.map, mapPar , Xtrain , Ytrain , numSamples , argin{:});
...
%                     norm(filter.weights)
                    ...
                    if obj.storeFullTrainPerf == 1         
                        ...
                        obj.trainPerformance(i , filter.currentParIdx) = trainPerf;
                    end
                    
                    if obj.storeFullValPerf == 1
                        obj.valPerformance(i , filter.currentParIdx) = valPerf;
                    end
                    if obj.storeFullTestPerf == 1      
                        ...
                        if obj.storeFullTestPred == 1
                            obj.testPred(i , filter.currentParIdx,:) = YtestPred;
                        end

                        % Compute training performance
                        testPerf = performanceMeasure( Yte , YtestPred , 1:size(Yte,1) );
                        
                        obj.testPerformance(i , filter.currentParIdx) = testPerf;                        
                    end
