classdef laplaceKernel < kernel
    ...
    methods
        ...
        function obj = laplaceKernel( X1 , X2 , numGuesses )
...
%             obj.rng = num2cell(tmp);
        ...
        function compute(obj , kerPar)
            ...
            if sum(sum(obj.SqDistMat < -10e-10)) > 0
                error('sqdistmat has neg vals!!'); 
            end
            
            if( nargin > 1 )
                obj.K = exp(-sqrt(abs(obj.SqDistMat)) / (2 * kerPar(1)));
            ...
            elseif (nargin == 1) && (isempty(obj.currentPar))
                ...
                obj.currentPar
                obj.K = exp(-sqrt(abs(obj.SqDistMat)) / (2 * obj.currentPar(1)));
            end
        end
