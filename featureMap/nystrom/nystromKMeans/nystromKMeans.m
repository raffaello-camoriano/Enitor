classdef nystromKMeans < nystrom
    % nystromKMeans Implementation of a Nystrom approximation of the Gram
    % matrix with selection of landmark points based on the k-means algorithm.
    %
    % Input parameters:
    ...
    methods
        % Constructor
        function obj = nystromKMeans( X , numNysParGuesses , numMapParGuesses , numMapParRangeSamples , minRank, maxRank , mapParGuesses , verbose)
...
%                 maxGuess = sqrt( max(D) );
        ...
        function compute(obj , mapPar)
            ...
            % Select landmark points by running  k-means
            [~ , obj.Xs] = kmeans(obj.X,chosenPar(1),'Replicates',1);
            ...
            obj.computeSqDistMat(obj.Xs , obj.Xs);
            obj.W = exp(-obj.SqDistMat / (2 * chosenPar(2)^2));
            obj.SqDistMat = [];     % Erase square distance matrix
        end
